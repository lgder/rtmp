...
int RtmpPublisher::SetMediaInfo(MediaInfo media_info) {
  ...
  if (media_info_.audio_codec_id == RTMP_CODEC_ID_AAC) {
    if (media_info_.audio_specific_config_size > 0) {
      ...
      // audio tag data
      audio_tag_ = data[0] = (((RTMP_CODEC_ID_AAC & 0xf) << 4) | ((sound_rate & 0x3) << 2) |
...
int RtmpPublisher::OpenUrl(std::string url, int msec, std::string &status) {
  ...
  if (rtmp_conn_ != nullptr) {
    ...
    task_scheduler_->AddTriggerEvent([rtmpConn]() { rtmpConn->Disconnect(); });
    rtmp_conn_ = nullptr;
  }
...
bool RtmpPublisher::IsKeyFrame(uint8_t *data, uint32_t size) {
  // H.264编码中，起始码可以是 00 00 01 或 00 00 00 01
  int startCode = 0;
  ...
  // 在H.264中，帧类型存储在第一个字节的5-7位
  int type = data[startCode] & 0x1f;
  if (type == 5 || type == 7) {  // 5: IDR帧  7: SPS帧
    return true;
  }
...
int RtmpPublisher::PushVideoFrame(uint8_t *data, uint32_t size) {
  ...
  if (media_info_.video_codec_id == RTMP_CODEC_ID_H264) {
    // 首帧数据必须是关键帧, 没有的话先检查当前帧是否关键帧, 是的话可以直接发
    if (!has_key_frame_) {
    ...
    uint32_t index = 0;
    buffer[index++] = this->IsKeyFrame(data, size) ? 0x17 : 0x27;  // 1: keyframe, 7: avc, 2: inter frame(非关键帧)
    buffer[index++] = 1;  // 1: avc raw data, 0: avc sequence header
